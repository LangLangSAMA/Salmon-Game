...
void Pebbles::collides_with_pebbles()
{
    ...
    for (auto &p1 : m_pebbles)
    {
        ...
        for (auto &p2 : m_pebbles)
        {
            ...
            {
                float adjust_dist = p1.radius + p2.radius - d;
                if (p1.position.y > p2.position.y)
                {
                    p2.position.y -= adjust_dist * y_diff / d;
                }
                else
                {
                    p2.position.y += adjust_dist * x_diff / d;
                }

                if (p1.position.x < p2.position.x)
                {
                    p2.position.x += adjust_dist * x_diff / d;
                }
                else
                {
                    p1.position.x += adjust_dist * x_diff / d;
                }
